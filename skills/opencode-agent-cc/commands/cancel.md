---
description: Cancel a running opencode process by PID
argument-hint: '[pid]'
disable-model-invocation: true
allowed-tools: Bash(node:*), Bash(ps:*), Bash(kill:*)
---

Stop an active `opencode` run by its PID.
...
3. If a PID was given: **verify it appears in the `running` list above** before doing anything. Never run
   `kill` on a PID that is not a confirmed opencode process from this list.
4. Terminate it: `kill <pid>`. Re-check with the probe; if it is still alive after a moment, escalate with
   `kill -9 <pid>`.
